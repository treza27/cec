...
export const convertToSupabase = (item: Partial<DepartItem>) => {
  return {
    ...
    ...(item.colisAssocies !== undefined && { colis_associes: item.colisAssocies })
  };
};
...
export const departureService = {
  ...
  async update(id: number, updates: Partial<DepartItem>): Promise<DepartItem> {
    ...
    // Si les colis associés changent, récupérer l'ancienne liste et le numBL avant la mise à jour
    let oldColisAssocies: number[] | null = null;
    if (updates.colisAssocies !== undefined || updates.numBL) {
      const { data: oldData } = await supabase
        .from('depart')
        .select('num_bl, colis_associes')
        .eq('id', id)
        .maybeSingle();
      oldColisAssocies = oldData?.colis_associes ?? null;
    }
    ...
    if (updates.colisAssocies !== undefined || updates.numBL) {
      ...
      // Retirer le BL uniquement des colis qui ont été explicitement retirés de la liste
      if (updates.colisAssocies !== undefined && oldColisAssocies) {
        const newIds = new Set(updates.colisAssocies);
        const removedIds = oldColisAssocies.filter((id: number) => !newIds.has(id));
