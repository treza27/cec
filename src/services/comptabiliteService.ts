...
export interface NoteDebitNonPayee {
  ...
  depart_id: number;
  depart: { id: number; num_bl: string; num_tc: string | null; date_arrivee_tana: string | null } | null;
}
...
export const noteDebitComptaService = {
  ...
  async getAll(): Promise<NoteDebitNonPayee[]> {
    const { data, error } = await supabase
      .from('notes_debit')
      .select('id, reference, client_pseudo, client_nom, montant_total_ariary, frais_livraison_ariary, statut_paiement, total_paye, volume_total_tana, created_at, apure_externe, depart_id, depart:depart_id(id, num_bl, num_tc, date_arrivee_tana)')
      .order('created_at', { ascending: false });
    if (error) throw error;
    return (data ?? []) as NoteDebitNonPayee[];
  },
...
export interface NoteDebitClient {
  ...
  apure_externe: boolean;
  created_at: string;
  poids_theorique_total: number;
  depart_id: number;
  depart_num_bl: string | null;
  depart_num_tc: string | null;
  depart_date_arrivee_tana: string | null;
}
...
export const releveClientService = {
  async getNotesDebitByPseudo(pseudo: string): Promise<NoteDebitClient[]> {
    const { data, error } = await supabase
      .from('notes_debit')
      .select('id, reference, client_pseudo, client_nom, montant_total_ariary, frais_livraison_ariary, volume_total_tana, prix_cbm_usd, statut_paiement, total_paye, apure_externe, created_at, colis_ids, depart_id, depart:depart_id(num_bl, num_tc, date_arrivee_tana)')
      .eq('client_pseudo', pseudo)
      .order('created_at', { ascending: false });
    if (error) throw error;

    const notes = (data ?? []) as (NoteDebitClient & { colis_ids: number[] | null; depart: { num_bl: string | null; num_tc: string | null; date_arrivee_tana: string | null } | null })[];

    const allColisIds = Array.from(new Set(notes.flatMap(n => n.colis_ids ?? [])));
    if (allColisIds.length === 0) {
      return notes.map(n => {
        const { colis_ids: _removed, depart: _depart, ...noteWithoutExtra } = n;
        return {
          ...noteWithoutExtra,
          poids_theorique_total: 0,
          depart_num_bl: n.depart?.num_bl ?? null,
          depart_num_tc: n.depart?.num_tc ?? null,
          depart_date_arrivee_tana: n.depart?.date_arrivee_tana ?? null,
        };
      });
    }
    ...
    const poidsMap = new Map<number, number>();
    for (const row of colisData ?? []) {
      poidsMap.set(row.id, Number(row.poids) || 0);
    }

    return notes.map(n => {
      const poidsTotal = (n.colis_ids ?? []).reduce((sum, id) => sum + (poidsMap.get(id) ?? 0), 0);
      const { colis_ids: _removed, depart: _depart, ...noteWithoutExtra } = n;
      return {
        ...noteWithoutExtra,
        poids_theorique_total: poidsTotal,
        depart_num_bl: n.depart?.num_bl ?? null,
        depart_num_tc: n.depart?.num_tc ?? null,
        depart_date_arrivee_tana: n.depart?.date_arrivee_tana ?? null,
      };
