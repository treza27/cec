...
export const deliveryReceiptService = {
  ...
  async uploadDeliveryReceipt(uploadData: DeliveryReceiptUpload): Promise<DeliveryReceipt> {
    try {
      ...
      throw new Error(`Erreur lors de l'upload du bon de livraison: ${error}`);
    }
  },
  ...
  async getDeliveryReceiptsByDepart(departId: number): Promise<DeliveryReceipt[]> {
    try {
      ...
      return (data || []).map(item => ({
        ...
        colis_ids: [], // À récupérer séparément si nécessaire
        client_name: item.clients ? `${item.clients.nom} ${item.clients.prenom}` : 'Client inconnu',
        shipping_marks: [] // À récupérer séparément si nécessaire
      }));
    } catch (error) {
      throw new Error(`Erreur lors de la récupération des bons de livraison: ${error}`);
    }
  },
  ...
  async deleteDeliveryReceipt(receiptId: string): Promise<void> {
    try {
      ...
      throw new Error(`Erreur lors de la suppression du bon de livraison: ${error}`);
    }
  },
  ...
  async getClientDeliveryGroups(departId: number, inventoryItems: InventoryItem[]): Promise<any[]> {
    try {
      // Récupérer le départ et ses colis associés
      ...
      // Récupérer les informations des clients via les shipping marks
      ...
      clientsData?.forEach(item => {
        ...
        if (!clientGroups[clientId]) {
          clientGroups[clientId] = {
            client_id: clientId,
            client_name: `${item.clients.nom} ${item.clients.prenom}`,
            shipping_marks: [],
            colis: [],
            delivery_receipts: []
          };
      ...
      // Récupérer les bons de livraison existants
        ...
        receiptsData.forEach(receipt => {
          if (receipt.client_id && clientGroups[receipt.client_id]) {
            clientGroups[receipt.client_id].delivery_receipts.push({
              ...
              colis_ids: [],
              client_name: clientGroups[receipt.client_id].client_name,
      ...
      throw new Error(`Erreur lors du groupement par client: ${error}`);
    }
  }
};