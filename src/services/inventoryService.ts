...
export const convertFromSupabase = (supabaseItem: SupabaseInventoryItem): InventoryItem => {
  return {
    ...
    numRecu: supabaseItem.num_recu || '',
    entrepot: supabaseItem.entrepot || '',
    shippingMark: supabaseItem.shipping_mark || '',
    description: supabaseItem.description,
    nbPalettes: (supabaseItem.nb_palettes || 0).toString(),
    nbCartons: (supabaseItem.nb_cartons || 1).toString(),
    poids: (supabaseItem.poids || 0).toString(),
    volume: (supabaseItem.volume || 0).toString(),
    nature: supabaseItem.nature || '',
    msds: supabaseItem.msds || false,
    images: [],
...
export const convertToSupabase = (item: Partial<InventoryItem>) => {
  return {
    ...
    ...(item.numRecu !== undefined && { num_recu: item.numRecu || null }),
    ...
    ...(item.poids !== undefined && { poids: item.poids && item.poids.trim() ? Math.max(0.1, parseFloat(item.poids)) : null }),
    ...(item.volume !== undefined && { volume: item.volume && item.volume.trim() ? Math.max(0.1, parseFloat(item.volume)) : null }),
    ...(item.nature !== undefined && { nature: item.nature || null }),
    ...(item.msds !== undefined && { msds: Boolean(item.msds) }),
    ...
    ...(item.poidsTana !== undefined && { poids_tana: item.poidsTana && item.poidsTana.trim() ? Math.max(0.1, parseFloat(item.poidsTana)) : null }),
    ...(item.volumeTana !== undefined && { volume_tana: item.volumeTana && item.volumeTana.trim() ? Math.max(0.1, parseFloat(item.volumeTana)) : null }),
...
export const inventoryService = {
    ...
    try {
      ...
      const { data: client, error: clientError } = await supabase
        ...
        .select(`
          ...
          entreprise,
          telephone,
          client_shipping_marks(shipping_mark, is_active)
        `)
        .ilike('pseudo', pseudo.trim()); // Utilisation de 'ilike' pour une recherche insensible à la casse
      ...
      console.log('✅ CLIENT TROUVÉ:', {
        ...
        shipping_marks_count: foundClient.client_shipping_marks?.length || 0
      });

      // Récupérer les shipping marks actives du client
      const activeShippingMarks = foundClient.client_shipping_marks
        .filter((mark: any) => mark.is_active)
        .map((mark: any) => mark.shipping_mark);

      console.log('🏷️ SHIPPING MARKS ACTIVES:', {
        total: activeShippingMarks.length,
        marks: activeShippingMarks
      });

      if (activeShippingMarks.length === 0) {
        console.log('❌ AUCUNE SHIPPING MARK ACTIVE pour ce client');
        console.log('❌ Vérifiez que le client a des shipping marks avec is_active = true');
        return [];
      }

      // Rechercher tous les colis avec ces shipping marks
      console.log('📦 Recherche des colis avec les shipping marks:', activeShippingMarks);
      const { data: packages, error: packageError } = await supabase
        .from('inventaire')
        .select('*')
        .in('shipping_mark', activeShippingMarks)
      ...
      return (packages || []).map(pkg => {
        const associatedDepart = departures?.find(depart =>
          ...
          depart.colis_associes?.includes(pkg.id)
        );
  ...
  async searchPackagesByClientInfo(clientInfo: { pseudo: string; phone: string }): Promise<InventoryItem[]> {
    try {
      ...
      console.log('🏷️ Shipping marks actives trouvées:', activeShippingMarks);
