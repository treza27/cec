...
export const inventoryService = {
  ...
  async getAll(): Promise<InventoryItem[]> {
    const data = await fetchAll<SupabaseInventoryItem>('inventaire', '*', {
      filter: (q) => q.neq('statut', 'archive'),
  ...
  async getGlobalStats(search: string): Promise<{ totalPalettes: number; totalCartons: number; totalPoids: number; totalVolume: number }> {
    const filterFn = (q: any) => {
      let query = q.neq('statut', 'archive');
      if (search.trim()) {
        const s = search.trim();
        query = query.or(
          `shipping_mark.ilike.%${s}%,description.ilike.%${s}%,bl.ilike.%${s}%,tracking_number.ilike.%${s}%,pseudo.ilike.%${s}%`
    ...
    const data = await fetchAll<{ nb_palettes: number | null; nb_cartons: number | null; poids: number | null; volume: number | null }>(
      'inventaire',
      'nb_palettes, nb_cartons, poids, volume',
      { filter: filterFn },
    );
  ...
  async searchPackagesByPseudo(pseudo: string): Promise<InventoryItem[]> {
    try {
      ...
      const packages = await fetchAll<SupabaseInventoryItem>('inventaire', '*', {
        filter: (q) => q.eq('pseudo', foundClient.pseudo),
        order: { column: 'created_at', ascending: false },
      });

      console.log('📦 RESULTAT COLIS:', {
        nombreColis: packages.length,
      ...
      console.log('✅ FINAL - Colis trouvés et enrichis:', packages.length);

      // Récupérer les informations des départs pour associer les numéros de conteneur
      const departures = await fetchAll<{ id: number; num_bl: string; num_tc: string | null; colis_associes: number[] | null; statut: string | null }>(
        'depart',
        'id, num_bl, num_tc, colis_associes, statut',
      );
  ...
  async searchPackagesByClientInfo(clientInfo: { pseudo: string; phone: string }): Promise<InventoryItem[]> {
    try {
      ...
      console.log('📦 Résultat de la recherche de colis:', { packages: packages.length });

      console.log('✅ Colis trouvés (incluant archivés):', packages.length);
  ...
  async searchByShippingMarkAndClient(shippingMark: string, clientInfo: { pseudo?: string; phone?: string }): Promise<InventoryItem[]> {
    try {
      ...
      const packages = await fetchAll<SupabaseInventoryItem>('inventaire', '*', {
        filter: (q) => q.in('shipping_mark', clientShippingMarks).neq('statut', 'archive'),
        order: { column: 'created_at', ascending: false },
      });
