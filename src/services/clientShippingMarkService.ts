...
export const clientShippingMarkService = {
  ...
  async getAllClientsWithShippingMarks(): Promise<ClientWithShippingMarks[]> {
    const data = await fetchAll<any>('clients', `*, client_shipping_marks(id, client_id, shipping_mark, is_active, created_at, updated_at)`, {
      filter: (q) => q.eq('client_shipping_marks.is_active', true),
