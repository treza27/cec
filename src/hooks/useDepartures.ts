...
export const useDepartures = () => {
  ...
  useEffect(() => {
    console.log('🔄 Configuration de la synchronisation temps réel pour les départs');
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('🚢 Changement détecté dans les départs:', payload);
      ...
      .on(
        'postgres_changes',
        {
          ...
          filter: 'depart_id=not.is.null'
        },
        (payload) => {
          console.log('📸 Changement détecté dans les images de départ:', payload);

          // Invalider et refetch les données des départs
          queryClient.invalidateQueries({ queryKey: departureKeys.lists(userId) });
        }
      )
      .subscribe((status) => {
        console.log('📡 Statut de la synchronisation départs:', status);
        if (status === 'CHANNEL_ERROR') {
          console.error('❌ Erreur de synchronisation temps réel pour les départs');
        }
      });

    // Nettoyage lors du démontage du composant
    return () => {
      console.log('🔌 Déconnexion de la synchronisation temps réel des départs');
...
export const useAllDepartures = () => {
  ...
  // Synchronisation en temps réel avec Supabase
  useEffect(() => {
    console.log('🔄 Configuration de la synchronisation temps réel pour tous les départs');
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('🚢 Changement détecté dans tous les départs:', payload);
      ...
      .subscribe((status) => {
        console.log('📡 Statut de la synchronisation tous départs:', status);
        if (status === 'CHANNEL_ERROR') {
          console.error('❌ Erreur de synchronisation temps réel pour tous les départs');
        }
      });

    // Nettoyage lors du démontage du composant
    return () => {
      console.log('🔌 Déconnexion de la synchronisation temps réel de tous les départs');
      supabase.removeChannel(channel);
    };
  }, [queryClient, userId]);
