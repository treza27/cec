...
import { Archive, Search, Calendar, Package, FileText, Download, Eye, Filter, RotateCcw, AlertTriangle, Weight, Truck, ChevronDown, ChevronUp, User, Building, Wallet, Building2, RotateCw } from 'lucide-react';
import toast from 'react-hot-toast';
import { useArchivedDepartures } from '../../hooks/useDepartures';
import { useAllInventoryItems } from '../../hooks/useInventory';
import { getDepartureStatusLabel, getDepartureStatusColor, getInventoryStatusColor, getEntrepotColor } from '../../utils/statusHelpers';
import { useCaissesArchivees, useRestoreCaisse, useComptesBancairesArchives, useRestoreCompteBancaire } from '../../hooks/useComptabilite';
...
export default function ArchivesPage() {
  ...
  const { data: caissesArchivees = [] } = useCaissesArchivees();
  const { data: comptesArchives = [] } = useComptesBancairesArchives();
  const restoreCaisse = useRestoreCaisse();
  const restoreCompte = useRestoreCompteBancaire();
  const [restoringId, setRestoringId] = useState<{ type: 'caisse' | 'compte'; id: number } | null>(null);
  ...
  const getColisForDepart = (departId: number) => {
    ...
    return inventoryItems.filter(item => idSet.has(Number(item.id)));
  };
  ...
  const handleRestoreCaisse = async (id: number) => {
    setRestoringId({ type: 'caisse', id });
    try {
      await restoreCaisse.mutateAsync(id);
      toast.success('Caisse restaurée');
    } catch (err) {
  ...
  const handleRestoreCompte = async (id: number) => {
    setRestoringId({ type: 'compte', id });
    try {
      await restoreCompte.mutateAsync(id);
      toast.success('Compte bancaire restauré');
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Erreur lors de la restauration');
    } finally {
      setRestoringId(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Caisses archivées */}
      {(caissesArchivees.length > 0 || comptesArchives.length > 0) && (
        <div className="space-y-4">
          ...
          {caissesArchivees.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="px-5 py-3 border-b bg-gray-50 flex items-center gap-2">
                <Wallet className="w-4 h-4 text-gray-500" />
                <span className="text-sm font-medium text-gray-700">Caisses</span>
                <span className="ml-auto text-xs text-gray-400">{caissesArchivees.length} archivée{caissesArchivees.length > 1 ? 's' : ''}</span>
              </div>
              <ul className="divide-y divide-gray-100">
                {caissesArchivees.map(c => (
                  <li key={c.id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50 transition-colors">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {c.type_caisse === 'especes' ? 'Espèces' : c.type_caisse === 'mvola' ? 'MVola' : 'Orange Money'}
                        {c.responsable && <span className="ml-2 text-gray-400">· {c.responsable.full_name}</span>}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-gray-400">
                        Archivé le {new Date(c.updated_at).toLocaleDateString('fr-FR')}
                      </span>
                      {isAdmin && (
                        <button
                          onClick={() => handleRestoreCaisse(c.id)}
                          disabled={restoringId?.type === 'caisse' && restoringId.id === c.id}
          ...
          {comptesArchives.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="px-5 py-3 border-b bg-gray-50 flex items-center gap-2">
                <Building2 className="w-4 h-4 text-gray-500" />
                <span className="text-sm font-medium text-gray-700">Comptes bancaires</span>
                <span className="ml-auto text-xs text-gray-400">{comptesArchives.length} archivé{comptesArchives.length > 1 ? 's' : ''}</span>
              </div>
              <ul className="divide-y divide-gray-100">
                {comptesArchives.map(c => (
                  <li key={c.id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50 transition-colors">
                    <div>
                      <p className="text-sm font-medium text-gray-800">{c.nom}</p>
                      <p className="text-xs text-gray-500">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {isAdmin && (
                        <button
                          onClick={() => handleRestoreCompte(c.id)}
                          disabled={restoringId?.type === 'compte' && restoringId.id === c.id}
                          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-green-700 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition-colors disabled:opacity-50"
                        >
                          <RotateCw className="w-3.5 h-3.5" />
                          Restaurer
