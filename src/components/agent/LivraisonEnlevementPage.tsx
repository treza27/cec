...
import { Truck, Package, ArrowLeft, Ship, Calendar, MapPin, Users, CreditCard as Edit, Save, X, FileText, Warehouse, Building, Receipt, ChevronDown, ChevronUp, Trash2, AlertTriangle, Calculator, TrendingUp, CircleDollarSign, RotateCw } from 'lucide-react';
...
export default function LivraisonEnlevementPage() {
  ...
  const [noteDebitStatutFilter, setNoteDebitStatutFilter] = useState<'all' | 'impayee' | 'partielle' | 'payee'>('all');
  ...
  const filteredNotesDebit = useMemo(() => {
    if (noteDebitStatutFilter === 'all') return notesDebit;
    return notesDebit.filter((n) => (n.statut_paiement || 'impayee') === noteDebitStatutFilter);
  }, [notesDebit, noteDebitStatutFilter]);

  const noteDebitStatutCounts = useMemo(() => ({
    all: notesDebit.length,
    impayee: notesDebit.filter((n) => (n.statut_paiement || 'impayee') === 'impayee').length,
    partielle: notesDebit.filter((n) => n.statut_paiement === 'partielle').length,
    payee: notesDebit.filter((n) => n.statut_paiement === 'payee').length,
  }), [notesDebit]);
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            {showNotesDebitHistory && (
              <div>
                ...
                <div className="flex flex-wrap gap-2 px-6 py-3 border-b border-gray-100 bg-white">
                  <button
                    onClick={() => setNoteDebitStatutFilter('all')}
                    className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${noteDebitStatutFilter === 'all' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                  >
                    Toutes ({noteDebitStatutCounts.all})
                  </button>
                  <button
                    onClick={() => setNoteDebitStatutFilter('impayee')}
                    className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${noteDebitStatutFilter === 'impayee' ? 'bg-red-500 text-white' : 'bg-red-50 text-red-700 hover:bg-red-100'}`}
                  >
                    Impayées ({noteDebitStatutCounts.impayee})
                  </button>
                  <button
                    onClick={() => setNoteDebitStatutFilter('partielle')}
                    className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${noteDebitStatutFilter === 'partielle' ? 'bg-amber-500 text-white' : 'bg-amber-50 text-amber-700 hover:bg-amber-100'}`}
                  >
                    Partielles ({noteDebitStatutCounts.partielle})
                  </button>
                  <button
                    onClick={() => setNoteDebitStatutFilter('payee')}
                    className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${noteDebitStatutFilter === 'payee' ? 'bg-green-500 text-white' : 'bg-green-50 text-green-700 hover:bg-green-100'}`}
                  >
                    Payées ({noteDebitStatutCounts.payee})
                  </button>
                </div>
                <div className="divide-y divide-gray-100">
                {notesDebitLoading ? (
                ...
                ) : filteredNotesDebit.length === 0 ? (
                  <div className="px-6 py-8 text-center">
                    <Receipt className="w-10 h-10 text-gray-300 mx-auto mb-3" />
                    <p className="text-sm text-gray-500">Aucune note de débit {noteDebitStatutFilter === 'impayee' ? 'impayée' : noteDebitStatutFilter === 'partielle' ? 'partielle' : noteDebitStatutFilter === 'payee' ? 'payée' : ''} pour ce départ.</p>
                  </div>
                ) : (
                  filteredNotesDebit.map((note) => (
                    <div key={note.id} className="px-6 py-4 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {note.nombre_modifications > 0 && (
                          <span
                            className="inline-flex items-center gap-1 px-2 py-1 text-xs font-bold rounded-full bg-amber-100 text-amber-700 border border-amber-300"
                            title={`Modifiée ${note.nombre_modifications} fois`}
                          >
                            <RotateCw className="w-3 h-3" />
                            {note.nombre_modifications}
                          </span>
                        )}
                        <button
