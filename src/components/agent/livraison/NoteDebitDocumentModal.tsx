...
import { X, Printer, Building2, RotateCw } from 'lucide-react';
...
export default function NoteDebitDocumentModal({
  ...
  const handlePrint = () => {
    ...
    const totalPoidsCM = det.reduce((sum, d) => sum + (d.poidsTana || 0), 0);
    const totalVolTheo = det.reduce((sum, d) => sum + (d.volumeTheorique ?? 0), 0);
    const totalPoidsTheo = det.reduce((sum, d) => sum + (d.poidsTheorique ?? 0), 0);
    const totalVolCM = det.reduce((sum, d) => sum + (d.volumeTana ?? 0), 0);
    const wfiTheorique = calculateWFI(String(totalVolTheo), String(totalPoidsTheo));
    const wfiContreMesure = calculateWFI(String(totalVolCM), String(totalPoidsCM));
    const useTheoVolume = noteDebit.volume_base === 'theorique';
    const totalVolDisplay = useTheoVolume ? totalVolTheo : totalVolCM;
    const totalPoidsDisplay = det.reduce((sum, d) => sum + ((d.poidsTana || 0) !== 0 ? (d.poidsTana || 0) : (d.poidsTheorique ?? 0)), 0);

    const rowsHtml = det.map((d, idx) => {
      const volDisplay = useTheoVolume ? (d.volumeTheorique ?? 0) : (d.volumeTana ?? 0);
      const poidsDisplay = (d.poidsTana || 0) !== 0 ? (d.poidsTana || 0) : (d.poidsTheorique ?? 0);
      return `
      <tr style="background:${idx % 2 === 0 ? '#ffffff' : '#f9fafb'};">
        <td style="border:1px solid #d1d5db;padding:8px 6px;text-align:center;color:#374151;font-size:10px;">${idx + 1}</td>
        <td style="border:1px solid #d1d5db;padding:8px 6px;font-weight:600;color:#1d4ed8;font-size:10px;white-space:nowrap;">${d.shippingMark || '—'}</td>
        <td style="border:1px solid #d1d5db;padding:8px 6px;color:#374151;font-size:9px;">${d.trackingNumber || '—'}</td>
        <td style="border:1px solid #d1d5db;padding:8px 6px;color:#374151;font-size:10px;">${d.description}</td>
        <td style="border:1px solid #d1d5db;padding:8px 6px;text-align:center;color:#1f2937;font-weight:500;font-size:10px;">${d.nbCartons || 0}</td>
        <td style="border:1px solid #d1d5db;padding:8px 6px;text-align:center;color:#1f2937;font-weight:500;font-size:10px;">${volDisplay.toFixed(3)}</td>
        <td style="border:1px solid #d1d5db;padding:8px 6px;text-align:center;font-weight:600;color:#111827;font-size:10px;">${poidsDisplay.toFixed(2)}</td>
      </tr>`;
    }).join('');

    const totalRowHtml = `
      <tr style="background-color:#1e40af;color:white;font-weight:700;">
        <td colspan="4" style="border:1px solid #1e3a8a;padding:8px 6px;text-align:right;font-size:10px;">TOTAL</td>
        <td style="border:1px solid #1e3a8a;padding:8px 6px;text-align:center;font-size:10px;">${totalCartons}</td>
        <td style="border:1px solid #1e3a8a;padding:8px 6px;text-align:center;font-size:10px;">${totalVolDisplay.toFixed(3)}</td>
        <td style="border:1px solid #1e3a8a;padding:8px 6px;text-align:center;font-size:10px;">${totalPoidsDisplay.toFixed(2)}</td>
      </tr>`;
...
<body>
      ...
      <table>
        <thead>
          <tr style="background-color:#1e40af;color:white;">
            <th style="border:1px solid #1e3a8a;padding:8px 6px;text-align:center;font-weight:600;width:36px;font-size:10px;">N°</th>
            <th style="border:1px solid #1e3a8a;padding:8px 6px;text-align:left;font-weight:600;width:140px;font-size:10px;white-space:nowrap;">Shipping Mark</th>
            <th style="border:1px solid #1e3a8a;padding:8px 6px;text-align:left;font-weight:600;width:90px;font-size:10px;">Tracking</th>
            <th style="border:1px solid #1e3a8a;padding:8px 6px;text-align:left;font-weight:600;font-size:10px;">Description</th>
            <th style="border:1px solid #1e3a8a;padding:8px 6px;text-align:center;font-weight:600;width:55px;font-size:10px;">Cartons</th>
            <th style="border:1px solid #1e3a8a;padding:8px 6px;text-align:center;font-weight:600;width:90px;font-size:10px;">Volume (m³)</th>
            <th style="border:1px solid #1e3a8a;padding:8px 6px;text-align:center;font-weight:600;width:90px;font-size:10px;">Poids (kg)</th>
          </tr>
        </thead>
    ...
    <div style="display:flex;justify-content:flex-end;padding-bottom:24px;page-break-inside:avoid;">
      <div style="width:300px;">
        <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid #e5e7eb;">
          <span style="font-size:11px;color:#4b5563;">Nombre de cartons</span>
          <span style="font-size:11px;font-weight:500;color:#1f2937;">${totalCartons}</span>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid #e5e7eb;">
          <span style="font-size:11px;color:#4b5563;">WFI</span>
          <span style="font-size:11px;font-weight:700;">
            <span style="color:${wfiTheorique === '-' ? '#6b7280' : (parseFloat(wfiTheorique) > 1 ? '#991b1b' : '#166534')}};">${wfiTheorique}</span>
            <span style="color:#9ca3af;"> / </span>
            <span style="color:${wfiContreMesure === '-' ? '#6b7280' : (parseFloat(wfiContreMesure) > 1 ? '#991b1b' : '#166534')}};">${wfiContreMesure}</span>
          </span>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid #e5e7eb;">
  ...
  const totalPoidsContreMesure = details.reduce((sum, d) => sum + (d.poidsTana || 0), 0);
  const totalVolCM = details.reduce((sum, d) => sum + (d.volumeTana ?? 0), 0);
  const totalPoidsTheo = details.reduce((sum, d) => sum + (d.poidsTheorique ?? 0), 0);
  const totalVolTheo = details.reduce((sum, d) => sum + (d.volumeTheorique ?? 0), 0);
  const wfiTheorique = calculateWFI(String(totalVolTheo), String(totalPoidsTheo));
  const wfiContreMesure = calculateWFI(String(totalVolCM), String(totalPoidsContreMesure));
  const useTheoVolume = noteDebit.volume_base === 'theorique';
  const totalVolDisplay = useTheoVolume ? totalVolTheo : totalVolCM;
  const totalPoidsDisplay = details.reduce((sum, d) => sum + ((d.poidsTana || 0) !== 0 ? (d.poidsTana || 0) : (d.poidsTheorique ?? 0)), 0);
        ...
        <div className="relative w-full max-w-4xl">
          <div className="no-print flex items-center justify-between mb-4">
            <span className="text-white font-semibold text-sm flex items-center gap-2">
              Note de Débit {noteDebit.reference}
              {noteDebit.nombre_modifications > 0 && (
                <span
                  className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-bold rounded-full bg-amber-400 text-amber-900"
                  title={`Modifiée ${noteDebit.nombre_modifications} fois`}
                >
                  <RotateCw className="w-3 h-3" />
                  {noteDebit.nombre_modifications}
                </span>
              )}
            </span>
              ...
              <table className="w-full text-xs" style={{ borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ backgroundColor: '#1e40af', color: 'white' }}>
                    <th className="px-3 py-2.5 font-semibold text-center" style={{ border: '1px solid #1e3a8a', width: '40px' }}>N°</th>
                    <th className="px-3 py-2.5 font-semibold text-left whitespace-nowrap" style={{ border: '1px solid #1e3a8a', width: '140px', whiteSpace: 'nowrap' }}>Shipping Mark</th>
                    <th className="px-3 py-2.5 font-semibold text-left" style={{ border: '1px solid #1e3a8a', width: '100px' }}>Tracking</th>
                    <th className="px-3 py-2.5 font-semibold text-left" style={{ border: '1px solid #1e3a8a' }}>Description</th>
                    <th className="px-3 py-2.5 font-semibold text-center" style={{ border: '1px solid #1e3a8a', width: '60px' }}>Cartons</th>
                    <th className="px-3 py-2.5 font-semibold text-center" style={{ border: '1px solid #1e3a8a', width: '95px' }}>Volume (m³)</th>
                    <th className="px-3 py-2.5 font-semibold text-center" style={{ border: '1px solid #1e3a8a', width: '95px' }}>Poids (kg)</th>
                  </tr>
                </thead>
                <tbody>
                  {details.map((detail, idx) => (
                    <tr key={detail.id} style={{ backgroundColor: idx % 2 === 0 ? 'white' : '#f9fafb' }}>
                      ...
                      <td className="px-3 py-3 font-semibold text-blue-700" style={{ border: '1px solid #d1d5db', whiteSpace: 'nowrap' }}>{detail.shippingMark || '—'}</td>
                      ...
                      <td className="px-3 py-3 text-center text-gray-800 font-medium text-xs" style={{ border: '1px solid #d1d5db' }}>
                        {(useTheoVolume ? ((detail as any).volumeTheorique ?? 0) : detail.volumeTana).toFixed(3)}
                      </td>
                      <td className="px-3 py-3 text-center text-gray-900 font-semibold text-xs" style={{ border: '1px solid #d1d5db' }}>
                        {((detail.poidsTana || 0) !== 0 ? (detail.poidsTana || 0) : ((detail as any).poidsTheorique ?? 0)).toFixed(2)}
                      </td>
                    </tr>
                  ))}
                    <tr style={{ backgroundColor: '#1e40af', color: 'white', fontWeight: 700 }}>
                      ...
                      <td className="px-3 py-3 text-center text-xs" style={{ border: '1px solid #1e3a8a' }}>
                        {totalVolDisplay.toFixed(3)}
                      </td>
                      <td className="px-3 py-3 text-center text-xs" style={{ border: '1px solid #1e3a8a' }}>
                        {totalPoidsDisplay.toFixed(2)}
                      </td>
                    </tr>
                </tbody>
              </table>
              ...
              <div className="flex justify-end">
                <div className="w-80">
                  <div className="flex items-center justify-between py-1.5 border-b border-gray-200">
                    <span className="text-xs text-gray-600">Nombre de cartons</span>
                    <span className="text-xs font-medium text-gray-800">{totalCartons}</span>
                  </div>
                  <div className="flex items-center justify-between py-1.5 border-b border-gray-200">
                    <span className="text-xs text-gray-600">WFI</span>
                    <span className="text-xs font-bold">
                      <span className={getWFIColor(wfiTheorique)}>{wfiTheorique}</span>
                      <span className="text-gray-400"> / </span>
                      <span className={getWFIColor(wfiContreMesure)}>{wfiContreMesure}</span>
                    </span>
                  </div>
                  <div className="flex items-center justify-between py-1.5 border-b border-gray-200">
