...
import { NoteDebit, NoteDebitCreateData, NoteDebitColisDetail, NoteDebitUpdateData, VolumeBase } from '../../../services/noteDebitService';
...
export default function NoteDebitConfigModal({
  ...
  const [volumeBase, setVolumeBase] = useState<VolumeBase>('contre_mesure');
  ...
  useEffect(() => {
    ...
    setVolumeBase(editNote.volume_base ?? 'contre_mesure');
  ...
  const volumeTheoriqueTotal = useMemo(
    () => selectedColis.reduce((sum, c) => sum + (Number(c.volume) || 0), 0),
    [selectedColis]
  );

  const poidsTheoriqueTotal = useMemo(
    () => selectedColis.reduce((sum, c) => sum + (Number(c.poids) || 0), 0),
    [selectedColis]
  );

  const poidsContreMesureTotal = useMemo(
    () => selectedColis.reduce((sum, c) => sum + (Number(c.poidsTana) || 0), 0),
    [selectedColis]
  );

  const volumeUsed = useMemo(
    () => volumeBase === 'theorique' ? volumeTheoriqueTotal : volumeTotal,
    [volumeBase, volumeTheoriqueTotal, volumeTotal]
  );

  const nbCartonsTotal = useMemo(
    () => selectedColis.reduce((sum, c) => sum + (Number(c.nbCartonsTana ?? c.nbCartons) || 0), 0),
    [selectedColis]
  );

  const wfiTheorique = useMemo(
    () => calculateWFI(String(volumeTheoriqueTotal), String(poidsTheoriqueTotal)),
    [volumeTheoriqueTotal, poidsTheoriqueTotal]
  );

  const wfiContreMesure = useMemo(
    () => calculateWFI(String(volumeTotal), String(poidsContreMesureTotal)),
    [volumeTotal, poidsContreMesureTotal]
  );

  const montantCbm = useMemo(() => {
    const prix = parseFloat(prixCbmUsd);
    const taux = parseFloat(tauxChange);
    if (!prix || !taux || !volumeUsed) return null;
    return prix * taux * volumeUsed;
  }, [prixCbmUsd, tauxChange, volumeUsed]);
  ...
  const handleGenerate = () => {
    ...
    const colisDetails: NoteDebitColisDetail[] = selectedColis.map((c) => {
      const volTana = Number(c.volumeTana) || 0;
      const volTheo = Number(c.volume) || 0;
      const vol = volumeBase === 'theorique' ? volTheo : volTana;
      return {
        id: c.id,
    ...
    if (isEditMode && editNote && onEdit) {
      const updateData: NoteDebitUpdateData = {
        prix_cbm_usd: prix,
        taux_change: taux,
        volume_total_tana: volumeBase === 'theorique' ? volumeTheoriqueTotal : volumeTotal,
          ...
          <div className="px-6 py-5">
            ...
            <div className="mb-6">
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                Volume utilisé pour le calcul
              </label>
              <div className="flex gap-3">
                <label
                  className={`flex-1 flex items-center gap-2.5 px-4 py-2.5 rounded-lg border cursor-pointer transition-colors ${volumeBase === 'theorique' ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-white hover:bg-gray-50'}`}
                >
                  <input
                    type="radio"
                    name="volumeBase"
                    checked={volumeBase === 'theorique'}
                    onChange={() => setVolumeBase('theorique')}
                    className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                  />
                  <span className={`text-sm font-medium ${volumeBase === 'theorique' ? 'text-blue-700' : 'text-gray-700'}`}>
                    Volume théorique
                  </span>
                  <span className={`ml-auto text-xs font-semibold ${volumeBase === 'theorique' ? 'text-blue-600' : 'text-gray-400'}`}>
                    {volumeTheoriqueTotal.toFixed(3)} m³
                  </span>
                </label>
                <label
                  className={`flex-1 flex items-center gap-2.5 px-4 py-2.5 rounded-lg border cursor-pointer transition-colors ${volumeBase === 'contre_mesure' ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-white hover:bg-gray-50'}`}
                >
                  <input
                    type="radio"
                    name="volumeBase"
                    checked={volumeBase === 'contre_mesure'}
                    onChange={() => setVolumeBase('contre_mesure')}
                    className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                  />
                  <span className={`text-sm font-medium ${volumeBase === 'contre_mesure' ? 'text-blue-700' : 'text-gray-700'}`}>
                    Volume contre-mesuré
                  </span>
                  <span className={`ml-auto text-xs font-semibold ${volumeBase === 'contre_mesure' ? 'text-blue-600' : 'text-gray-400'}`}>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-5">
              ...
              <div className="space-y-2.5">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Volume théorique / Volume contre-mesuré</span>
                  <span className="font-medium text-gray-900">
                    <span className={volumeBase === 'theorique' ? 'text-blue-700 font-bold' : ''}>{volumeTheoriqueTotal.toFixed(3)} m³</span>
                    <span className="text-gray-400"> / </span>
                    <span className={volumeBase === 'contre_mesure' ? 'text-blue-700 font-bold' : ''}>{volumeTotal.toFixed(3)} m³</span>
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Poids théorique / Poids contre-mesuré</span>
                  <span className="font-medium text-gray-900">
                    {poidsTheoriqueTotal.toFixed(2)} kg <span className="text-gray-400">/</span> {poidsContreMesureTotal.toFixed(2)} kg
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Nombre de cartons</span>
                  <span className="font-medium text-gray-900">{nbCartonsTotal}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">WFI</span>
                  <span className="font-medium">
                    <span className={`font-bold ${getWFIColor(wfiTheorique)}`}>{wfiTheorique}</span>
                    <span className="text-gray-400"> / </span>
                    <span className={`font-bold ${getWFIColor(wfiContreMesure)}`}>{wfiContreMesure}</span>
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
              ...
              {montantCbm != null && (
                <p className="text-xs text-gray-400 mt-3 italic">
                  Formule : {volumeUsed.toFixed(3)} m³ ({volumeBase === 'theorique' ? 'théorique' : 'contre-mesuré'}) × {parseFloat(prixCbmUsd).toFixed(2)} USD × {parseInt(tauxChange).toLocaleString('fr-FR')} Ar/USD
                </p>
              )}
            </div>
