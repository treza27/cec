...
const MODE_PAIEMENT_STYLES: Record<string, string> = {
  especes: 'bg-emerald-50 text-emerald-700 border-emerald-200',
  cheque: 'bg-amber-50 text-amber-700 border-amber-200',
  virement: 'bg-sky-50 text-sky-700 border-sky-200',
  mvola: 'bg-orange-50 text-orange-700 border-orange-200',
  orange_money: 'bg-red-50 text-red-700 border-red-200',
};
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">
                      {m.mode_paiement ? (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${MODE_PAIEMENT_STYLES[m.mode_paiement] ?? 'bg-gray-50 text-gray-600 border-gray-200'}`}>
            ...
            {!isEditing && (
              <>
                ...
                <div className="flex flex-col">
                  ...
                  {m.mode_paiement ? (
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${MODE_PAIEMENT_STYLES[m.mode_paiement] ?? 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                      {MODE_PAIEMENT_LABELS[m.mode_paiement] ?? m.mode_paiement}
                    </span>
...
function TabNotesDebit() {
  ...
  const filtered = useMemo(() => {
    const q = filtreDepart.trim().replace(/^#/, '').toLowerCase();
    if (!q) return notes;
    return notes.filter(n => n.depart_id != null && String(n.depart_id) === q);
  }, [notes, filtreDepart]);
  ...
  return (
    <div className="space-y-4">
      {notes.length === 0 ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-5 py-3.5 border-b bg-gray-50 flex items-center gap-3 flex-wrap">
            <p className="text-sm font-semibold text-gray-700">Notes de débit ({filtered.length})</p>
            <div className="flex items-center gap-2 ml-auto">
              ...
              <input
                ...
                placeholder="Filtrer par n° départ (ex: 21)"
                className="border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-700 bg-white focus:ring-2 focus:ring-blue-500 focus:border-transparent w-48"
              />
              {filtreDepart && (
                <button
                  onClick={() => setFiltreDepart('')}
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-8 text-sm text-gray-400">
                      Aucune note de débit pour le départ #{filtreDepart.replace(/^#/, '')}
                    </td>
                  </tr>
                ) : filtered.map((n) => {
                  ...
                  return (
                    <tr key={n.id} className={rowClass(n.statut_paiement)}>
                      <td
                        className="px-3 py-3 text-sm font-semibold text-gray-800 whitespace-nowrap cursor-help"
                        title={n.depart ? `BL: ${n.depart.num_bl || '—'}\nTC: ${n.depart.num_tc || '—'}\nArrivée Tana: ${n.depart.date_arrivee_tana ? formatDate(n.depart.date_arrivee_tana) : '—'}` : undefined}
                      >
                        {n.depart ? `#${n.depart.id}` : '—'}
                      </td>
                      ...
                      <td className="px-3 py-3 text-right">
                        <span className={`text-sm font-bold ${reste === 0 ? 'text-green-700' : n.statut_paiement === 'partielle' ? 'text-amber-700' : 'text-red-600'}`}>
                          {formatMga(reste)}
                        </span>
                      </td>
