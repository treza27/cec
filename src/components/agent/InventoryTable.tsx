...
import { Search, Edit, Check, X, Eye, ChevronLeft, ChevronRight } from 'lucide-react';
        ...
        <table className="w-full min-w-max">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-2 sm:px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Nature</th>
              <th className="px-2 sm:px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">MSDS</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {inventoryItems.map((item) => (
              <tr key={item.id} className={editingId === item.id ? 'bg-blue-50' : ''}>
                ...
                <td className="px-2 sm:px-3 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                  {editingId === item.id ? renderEditableCell('numRecu', editingItem?.numRecu || item.numRecu) : (item.numRecu || '-')}
                </td>
                ...
                <td className="px-2 sm:px-3 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                  {editingId === item.id ? renderEditableCell('volume', editingItem?.volume || item.volume, 'number') : `${item.volume} m³`}
                </td>
                ...
                <td className="px-2 sm:px-3 py-4 whitespace-nowrap text-xs sm:text-sm">
                  {editingId === item.id ? renderEditableCell('nature', editingItem?.nature || item.nature, 'select', ['GG', 'SG', 'DG']) : (
                    item.nature ? (
                ...
                <td className="px-2 sm:px-3 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                  {editingId === item.id ? renderEditableCell('msds', editingItem?.msds !== undefined ? editingItem.msds : item.msds, 'checkbox') : (
                    item.msds ? (
                      <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                        Oui
                      </span>
            ...
            {isAddingNew && (
              <tr className="bg-green-50 border-2 border-green-200">
                ...
                <td className="px-2 sm:px-3 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                  {renderEditableCell('numRecu', newItem.numRecu, 'text', undefined, true)}
                </td>
                <td className="px-2 sm:px-3 py-4 whitespace-nowrap text-xs sm:text-sm">
                ...
                <td className="px-2 sm:px-3 py-4 whitespace-nowrap text-xs sm:text-sm">
                  {renderEditableCell('nature', newItem.nature, 'select', ['GG', 'SG', 'DG'], true)}
                </td>
                <td className="px-2 sm:px-3 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                  {renderEditableCell('msds', newItem.msds, 'checkbox', undefined, true)}
                </td>
                <td className="px-2 sm:px-3 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
