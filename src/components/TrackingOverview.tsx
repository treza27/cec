...
import { ArrowLeft, Package, Calendar, MapPin, Weight, Truck, Eye, CheckCircle, Clock, AlertTriangle, Ship, Users, Image as ImageIcon, ChevronLeft, ChevronRight, X, Archive, FileText, Hash, Wallet } from 'lucide-react';
...
import ClientReleveFinancier from './ClientReleveFinancier';
...
export default function TrackingOverview({ packages, pseudo, onSelectDeparture, onSelectNoDepartureGroup, onBack }: TrackingOverviewProps) {
  ...
  const [activeTab, setActiveTab] = React.useState<'active' | 'archived' | 'documents' | 'releve'>('active');
  ...
  const groupPackagesByDeparture = (): DepartureGroup[] => {
    ...
    packages.forEach(pkg => {
      ...
      const pkgPoids = parseFloat(pkg.poids);
      const pkgVolume = parseFloat(pkg.volume);
      const pkgPalettes = parseInt(pkg.nbPalettes);
      if (!isNaN(pkgPoids) && pkgPoids > 0) {
        groups[groupKey].totalWeight += pkgPoids;
        groups[groupKey].hasConfirmedWeight = true;
      }
      if (!isNaN(pkgVolume) && pkgVolume > 0) {
        groups[groupKey].totalVolume += pkgVolume;
        groups[groupKey].hasConfirmedVolume = true;
      }
      if (!isNaN(pkgPalettes) && pkgPalettes > 0) {
        groups[groupKey].totalPallets += pkgPalettes;
        groups[groupKey].hasConfirmedPallets = true;
      }
  ...
  return (
    <section className="py-12 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Onglets pour séparer les départs actifs, archivés et documents */}
        <div className="mb-6">
              ...
              {pseudo && (
                <>
                  ...
                  >
                    ...
                    <span className="text-sm sm:text-base">Mes documents</span>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <span className="text-sm sm:text-base">Relevé financier</span>
                  </button>
                </>
              )}
            </div>
        ...
        {/* Documents client */}
        {activeTab === 'documents' && pseudo && (
          <ClientDocumentsList pseudo={pseudo} />
        )}

        {/* Relevé financier client */}
        {activeTab === 'releve' && pseudo && (
          <ClientReleveFinancier pseudo={pseudo} />
        )}

        {/* Avertissement approximation poids/volumes */}
        {activeTab !== 'documents' && activeTab !== 'releve' && (
          <div className="flex items-start gap-3 bg-red-50 border border-red-300 rounded-xl px-4 py-3">
            <AlertTriangle className="w-5 h-5 text-red-600 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-bold text-red-700">
                Les poids et volumes affichés sont des approximations.
              </p>
              <p className="text-sm text-red-600 mt-0.5">
                Les mesures exactes seront effectuées a l'arrivee a Antananarivo et serviront de base au calcul de votre note de debit finale.
              </p>
            </div>
          </div>
        )}
                ...
                <div className="p-3 sm:p-4 bg-gradient-to-r from-gray-50 to-blue-50/30 border-b border-gray-100">
                  <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                    ...
                    <div className="hidden xl:flex items-center gap-4 flex-shrink-0">
                      <div className={`flex items-center gap-1 px-2 py-1 rounded-lg ${group.hasConfirmedPallets ? 'bg-blue-50' : 'bg-gray-50'}`}>
                        <Package className={`w-3.5 h-3.5 ${group.hasConfirmedPallets ? 'text-blue-600' : 'text-gray-400'}`} />
                        {group.hasConfirmedPallets ? (
                          <>
                            <span className="text-xs font-bold text-blue-700">{group.totalPallets}</span>
                            <span className="text-xs text-gray-600">pal.</span>
                          </>
                        ) : (
                          <span className="text-xs italic text-gray-500">A confirmer</span>
                        )}
                      </div>
                      ...
                      <div className={`flex items-center gap-1 px-2 py-1 rounded-lg ${group.hasConfirmedWeight ? 'bg-green-50' : 'bg-gray-50'}`}>
                        <Weight className={`w-3.5 h-3.5 ${group.hasConfirmedWeight ? 'text-green-600' : 'text-gray-400'}`} />
                        {group.hasConfirmedWeight ? (
                          <span className="text-xs font-bold text-green-700">{group.totalWeight.toFixed(1)}kg</span>
                        ) : (
                          <span className="text-xs italic text-gray-500">A confirmer</span>
                        )}
                      </div>
                      <div className={`flex items-center gap-1 px-2 py-1 rounded-lg ${group.hasConfirmedVolume ? 'bg-sky-50' : 'bg-gray-50'}`}>
                        <Truck className={`w-3.5 h-3.5 ${group.hasConfirmedVolume ? 'text-sky-600' : 'text-gray-400'}`} />
                        {group.hasConfirmedVolume ? (
                          <span className="text-xs font-bold text-sky-700">{group.totalVolume.toFixed(1)}m³</span>
                        ) : (
                          <span className="text-xs italic text-gray-500">A confirmer</span>
                        )}
                      </div>
                  ...
                  <div className="xl:hidden mt-3 pt-3 border-t border-gray-200">
                    <div className="grid grid-cols-4 gap-2">
                      <div className={`rounded-lg p-2 ${group.hasConfirmedPallets ? 'bg-blue-50' : 'bg-gray-50'}`}>
                        <div className="flex items-center justify-center gap-1 mb-0.5">
                          <Package className={`w-3 h-3 ${group.hasConfirmedPallets ? 'text-blue-600' : 'text-gray-400'}`} />
                          {group.hasConfirmedPallets && <span className="text-xs font-bold text-blue-700">{group.totalPallets}</span>}
                        </div>
                        <span className="text-xs text-gray-600 block text-center">{group.hasConfirmedPallets ? 'Pal.' : '?'}</span>
                      </div>
                      ...
                      <div className={`rounded-lg p-2 ${group.hasConfirmedWeight ? 'bg-green-50' : 'bg-gray-50'}`}>
                        <div className="flex items-center justify-center gap-1 mb-0.5">
                          <Weight className={`w-3 h-3 ${group.hasConfirmedWeight ? 'text-green-600' : 'text-gray-400'}`} />
                          {group.hasConfirmedWeight && <span className="text-xs font-bold text-green-700">{group.totalWeight.toFixed(1)}</span>}
                        </div>
                        <span className="text-xs text-gray-600 block text-center">{group.hasConfirmedWeight ? 'kg' : '?'}</span>
                      </div>
                      <div className={`rounded-lg p-2 ${group.hasConfirmedVolume ? 'bg-sky-50' : 'bg-gray-50'}`}>
                        <div className="flex items-center justify-center gap-1 mb-0.5">
                          <Truck className={`w-3 h-3 ${group.hasConfirmedVolume ? 'text-sky-600' : 'text-gray-400'}`} />
                          {group.hasConfirmedVolume && <span className="text-xs font-bold text-sky-700">{group.totalVolume.toFixed(1)}</span>}
                        </div>
                        <span className="text-xs text-gray-600 block text-center">{group.hasConfirmedVolume ? 'm³' : '?'}</span>
                      </div>
                    </div>
                  </div>
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            {/* Deuxième ligne : Description + Date d'entrée */}
                            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                              <div className="text-xs sm:text-sm text-gray-600 leading-relaxed flex-1">
                                {packageItem.description}
                              </div>
                              {packageItem.dateEntree && (
                                <div className="flex items-center gap-1.5 text-xs text-gray-500 bg-gray-50 px-2 py-1 rounded-md border border-gray-200 whitespace-nowrap flex-shrink-0">
                                  <Calendar className="w-3 h-3" />
                                  <span className="font-medium">Entrée:</span>
                                  <span className="font-semibold text-gray-700">
                                    {new Date(packageItem.dateEntree).toLocaleDateString('fr-FR')}
                                  </span>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-1.5 flex-wrap">
                              {(() => {
                                const palVal = packageItem.nbPalettes;
                                const poidsVal = packageItem.poids;
                                const volumeVal = packageItem.volume;
                                const palEmpty = !palVal || palVal === '' || palVal === '0';
                                const poidsEmpty = !poidsVal || poidsVal === '' || parseFloat(poidsVal) === 0;
                                const volumeEmpty = !volumeVal || volumeVal === '' || parseFloat(volumeVal) === 0;
                                return (
                                  <>
                                    <span className={`px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap ${palEmpty ? 'bg-gray-100 text-gray-500 italic' : 'bg-blue-100 text-blue-700'}`}>
                                      {palEmpty ? 'A confirmer' : `${palVal} pal.`}
                                    </span>
                                    ...
                                    <span className={`px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap ${poidsEmpty ? 'bg-gray-100 text-gray-500 italic' : 'bg-green-100 text-green-700'}`}>
                                      {poidsEmpty ? 'A confirmer' : `${poidsVal} kg`}
                                    </span>
                                    <span className={`px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap ${volumeEmpty ? 'bg-gray-100 text-gray-500 italic' : 'bg-sky-100 text-sky-700'}`}>
                                      {volumeEmpty ? 'A confirmer' : `${volumeVal} m³`}
                                    </span>
                                  </>
                                );
                              })()}
                            ...
                            {/* Tracking number — affiché uniquement si renseigné */}
                            {packageItem.trackingNumber && (
                              <div className="flex items-center gap-1.5 mt-0.5">
                                <Hash className="w-3 h-3 text-indigo-500 flex-shrink-0" />
                                <span className="text-xs text-gray-500 font-medium">N° de suivi :</span>
                                <span className="text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded select-all">
                                  {packageItem.trackingNumber}
                                </span>
                              </div>
                            )}
