...
export default function DepartureForm({
  ...
  const stableSelectedColisIds = useMemo(() => selectedColisIds, [selectedColisIds.join(',')]);
  ...
  const allColis = useMemo(() => {
    ...
    extraColis.forEach(item => { if (!map.has(Number(item.id))) map.set(Number(item.id), item); });
    return Array.from(map.values());
  }, [inventoryItems, extraColis]);

  useEffect(() => {
    if (selectedColisIds.length === 0 || inventoryItems.length === 0) return;
    const inventoryIdSet = new Set(inventoryItems.map(item => Number(item.id)));
    const missingIds = selectedColisIds.filter(id => !inventoryIdSet.has(Number(id)));
    if (missingIds.length === 0) { setExtraColis([]); return; }
    inventoryService.getByIds(missingIds).then(setExtraColis).catch(() => setExtraColis([]));
  }, [selectedColisIds.join(','), inventoryItems.length]);

  useEffect(() => {
    if (allColis.length > 0 && selectedColisIds.length > 0) {
      const stats = calculateSelectionStats(selectedColisIds, allColis);
      setColisStats(stats);
    }
  }, [allColis, selectedColisIds]);
  ...
  const handleFormSubmit = (data: DepartureFormData) => {
    const statsAreEmpty = colisStats.nbPalettes === 0 && colisStats.nbCartons === 0 && colisStats.poids === 0 && colisStats.volume === 0;
    const hasExistingTotals = initialData && (initialData.nbPalettesTotal || initialData.nbCartonsTotal || initialData.poidsTotal || initialData.volumeTotal);
    const useExistingTotals = statsAreEmpty && selectedColisIds.length > 0 && hasExistingTotals;

    const formDataWithTotals = {
      ...data,
      nbPalettesTotal: useExistingTotals ? initialData!.nbPalettesTotal! : colisStats.nbPalettes,
      nbCartonsTotal: useExistingTotals ? initialData!.nbCartonsTotal! : colisStats.nbCartons,
      poidsTotal: useExistingTotals ? initialData!.poidsTotal! : colisStats.poids,
      volumeTotal: useExistingTotals ? initialData!.volumeTotal! : colisStats.volume,
      colisAssocies: selectedColisIds
    };
    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {selectedColisIds.map(colisId => {
                  const colis = allColis.find(item => Number(item.id) === Number(colisId));
        ...
        <PackageSelectionModal
          ...
          inventoryItems={allColis.length > inventoryItems.length ? allColis : inventoryItems}
          initialSelectedIds={stableSelectedColisIds}
          departureKey={initialData?.id ? `depart_${initialData.id}` : `depart_new_${watch('numBL') || 'draft'}`}
          lockedIds={initialData?.colisAssocies || []}
          isAdmin={isAdmin}
        />
