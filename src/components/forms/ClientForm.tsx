...
import { Save, X, Tag, User, Plus } from 'lucide-react';
...
export default function ClientForm({
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        statut_contact: (initialData as any).statut_contact || 'Prospect',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formattedData: ClientFormData & { statut_contact: StatutContact } = {
        ...formData,
        ...
        statut_contact: formData.statut_contact
      };
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            placeholder="Saisir un pseudo unique (min 3 caractères)"
            disabled={isSubmitting}
          />
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            placeholder="+33 6 12 34 56 78 ou +261 34 12 345 67"
            disabled={isSubmitting}
          />
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {(['Prospect', 'Client Argent', 'Client Or', 'Client Platine'] as StatutContact[]).map(s => {
              const colors: Record<StatutContact, string> = {
                'Prospect': 'border-gray-300 bg-gray-50 text-gray-700 data-[selected=true]:border-gray-500 data-[selected=true]:bg-gray-100',
                'Client Argent': 'border-slate-300 bg-slate-50 text-slate-700 data-[selected=true]:border-slate-500 data-[selected=true]:bg-slate-100',
                'Client Or': 'border-amber-300 bg-amber-50 text-amber-700 data-[selected=true]:border-amber-500 data-[selected=true]:bg-amber-100',
                'Client Platine': 'border-sky-300 bg-sky-50 text-sky-700 data-[selected=true]:border-sky-500 data-[selected=true]:bg-sky-100',
              };
              const dots: Record<StatutContact, string> = {
                'Prospect': 'bg-gray-400',
                'Client Argent': 'bg-slate-400',
                'Client Or': 'bg-amber-400',
                'Client Platine': 'bg-sky-500',
              };
              const labels: Record<StatutContact, string> = {
                'Prospect': 'Prospect',
                'Client Argent': 'Argent',
                'Client Or': 'Or',
                'Client Platine': 'Platine',
              };
              const selected = formData.statut_contact === s;
              return (
                <button
                  key={s}
                  type="button"
                  onClick={() => setFormData(prev => ({ ...prev, statut_contact: s }))}
                  disabled={isSubmitting}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg border-2 text-sm font-medium transition-all ${
                    selected
                      ? `${colors[s].split(' ').filter(c => c.startsWith('data-[selected=true]:bg-') || c.startsWith('data-[selected=true]:border-')).map(c => c.replace('data-[selected=true]:', '')).join(' ')} ring-2 ring-offset-1 ring-blue-400`
                      : colors[s].split(' ').filter(c => !c.startsWith('data-[selected=true]:')).join(' ')
                  } disabled:opacity-50 disabled:cursor-not-allowed`}
                >
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 ${dots[s]}`} />
                  {labels[s]}
                </button>
              );
            })}
          ...
          <p className="text-xs text-gray-500 mt-1">
            Par défaut : Prospect. Argent est attribué automatiquement au premier achat.
          </p>
        </div>
