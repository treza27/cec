...
import { X, Package, Weight, Truck, Check, Search, ArrowUpDown, Hash, Clock, Lock } from 'lucide-react';
...
export default function PackageSelectionModal({
  ...
  const lockedIdSet = useMemo(() => new Set(lockedIds.map(Number)), [lockedIds]);
  ...
  const handleToggleSelection = (colisId: number) => {
    ...
    if (currentlySelected && lockedIdSet.has(numId) && !isAdmin) {
      toast.error('Ce colis est déjà confirmé. Seul un administrateur peut le retirer du départ.');
      return;
    }
    setSelectedIds(prev =>
      currentlySelected
  ...
  const handleDeselectAll = () => {
    if (!isAdmin && lockedIdSet.size > 0) {
      setSelectedIds(prev => prev.filter(id => lockedIdSet.has(Number(id))));
      toast.error('Les colis déjà confirmés ne peuvent pas être désélectionnés. Seul un administrateur peut le faire.');
      return;
    }
    setSelectedIds([]);
  };
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredAndSortedItems.map((colis) => {
                  const isLocked = lockedIdSet.has(Number(colis.id)) && !isAdmin;
                  return (
                  <tr
                    key={colis.id}
                    ...
                    onClick={() => !isLocked && handleToggleSelection(colis.id)}
                  >
